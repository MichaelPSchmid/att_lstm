...
class DotAttention(nn.Module):
    ...
    def forward(self, x):
        """
        Self-attention mechanism without changing the hidden_size dimension.
        
        Args:
            x (tensor): Shape (batch, seq_len, hidden_size)

        Returns:
            tensor: Shape (batch, seq_len, hidden_size) after applying attention
        """
        e = torch.bmm(x, x.permute(0, 2, 1))  # Compute attention scores (b, s, h) * (b, h, s) -> (b, s, s)
        attention = F.softmax(e, dim=-1)  # Normalize attention scores
        out = torch.bmm(attention, x)  # Weighted sum (b, s, s) * (b, s, h) -> (b, s, h)
        return F.relu(out)  # Apply ReLU activation
...
class LSTMDotAttentionModel(pl.LightningModule):
    def __init__(self, input_size, hidden_size, num_layers, output_size, lr=0.001):
        ...
        # Self-attention layer (keeps hidden_size unchanged)
    ...
    def forward(self, x):
        ...
        att_output = self.attention(lstm_output)  # Apply attention (batch, seq_len, hidden_size)

        output = self.fc(att_output[:, -1, :])  # Use last time step features for final prediction
